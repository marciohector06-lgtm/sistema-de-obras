...
## Fase 11 — Cadastros e Design (concluída)

### Cadastros
- **Cliente**: CPF/CNPJ e endereço já existiam no schema/Zod mas nunca apareciam no formulário (só nome/e-mail/telefone) — adicionados ao dialog inline (`ClienteSelect.tsx`).
- **Prestador**: adicionados `email`, `telefone` (não existia nenhum jeito de contatar um prestador antes) e `categoria` virou enum (`PrestadorCategoria`: mão de obra geral, elétrica, hidráulica, pintura, alvenaria, acabamento, outro) em vez de texto livre — evita divergência de grafia. Labels centralizados em `lib/prestadores.ts`.
- **Obra**: adicionados `tipo` (enum `TipoObra`: residencial/comercial/reforma/institucional/outro), `cidade` e `responsavelTecnico`, todos opcionais para não pesar o cadastro. Exibidos no painel "Detalhes" da página da obra.
- **Convite de usuário**: não existia nenhum jeito do admin criar/convidar usuário — só autocadastro público em `/registro`. Novo `POST /api/usuarios` (admin-only) usa `supabase.auth.admin.inviteUserByEmail` (mesmo padrão do `prisma/seed.ts`) + cria a linha `User` (`PENDING`). Modal "Convidar usuário" em `/usuarios`.
- **Bug real corrigido**: o autocadastro em `/registro` criava o usuário no Supabase Auth mas **nunca criava a linha correspondente em `User`** — quem se cadastrava sozinho não conseguia usar o sistema. `getSessionUser()` (`lib/auth.ts`) agora autoprovisiona a linha (`VIEWER`/`PENDING`) no primeiro acesso autenticado, se ainda não existir.
- **Proposta**: adicionados `validade` (data) e `condicoesPagamento` (texto livre), impressos no PDF quando presentes.

### Design
- **Tendência nos KPIs**: `KpiCard` já suportava `variacao` mas nunca era usado — dashboard agora mostra variação de gasto mês a mês, com seta (↑/↓) refletindo a **direção real do número**, não o sentimento da cor (bug corrigido durante o teste: seta pra cima com texto "-100%" ficava contraditório).
- **Gráfico agregado no dashboard**: novo card "Gasto Total por Mês" (soma de todas as obras), reaproveitando o `GastosChart`/Recharts já usado na página de obra individual (`agruparGastosPorMes` em `lib/gastos.ts`).
- **Bug real corrigido**: o dashboard calculava os KPIs (portfólio/gasto/saldo) só a partir das 8 obras mais recentes (`take: 8`, usado pra lista "Progresso das Obras") — os totais ficariam incompletos/errados a partir da 9ª obra. Os KPIs agora vêm de uma query separada sem limite; a lista de 8 continua igual.
- **Busca global (Cmd/Ctrl+K)**: `components/shared/CommandPalette.tsx`, busca em Obras e Clientes (`/api/clientes` ganhou o filtro `?q=` que só `/api/obras` tinha).
- **Itens mortos do menu**: "Configurações" agora aponta para `/configuracoes` (rota já existia); "Meu perfil" ganhou uma página real (`/perfil`); "Sair" **não fazia nada** (nenhum `signOut` existia no projeto inteiro) — corrigido para chamar `supabase.auth.signOut()` de verdade.
- **Bug real corrigido**: o layout autenticado (`app/(dashboard)/layout.tsx`) passava `userName="Márcio Héctor"` e `userRole="ADMIN"` fixos no Header/Sidebar pra **qualquer** usuário logado — agora usa `getSessionUser()`.
- **Marca consistente**: sidebar mostrava "BiddingTech", header mostrava "Fornax Engenharia" — unificado em `NEXT_PUBLIC_APP_NAME="Fornax Engenharia"`, usado em Sidebar/Header/título da página/login/registro.
- **Estados vazios**: extraído `components/shared/EmptyState.tsx` do padrão que já existia (bom) em Obras/Propostas, aplicado nos lugares que só tinham texto simples (usuários, financeiro, inventário, pagamentos, prestadores, materiais, alertas).
- **Loading skeleton**: `loading.tsx` (convenção do App Router) adicionado para `/dashboard` e `/obras`.
- **Modo escuro**: `next-themes` estava instalado mas nunca usado — `ThemeProvider` em `app/layout.tsx`, paleta `.dark` em `globals.css`, botão de alternância (sol/lua) no Header.
- **Bug real corrigido (fora do escopo do design, achado durante o teste)**: os `<Select>` de "Tipo de obra" e "Categoria" ficavam com o gatilho **em branco** quando nenhum valor estava selecionado — o `placeholder` da `SelectValue` só funciona se a função de render devolver esse texto explicitamente quando o valor é vazio (padrão já usado em `ClienteSelect.tsx`, mas não replicado nos dois selects novos até o teste apontar o problema).
- **Bug de timezone encontrado (corrigido só na `validade` da proposta, não em todo o app)**: `new Date(e.target.value)` num input de data (`YYYY-MM-DD`) é interpretado como meia-noite UTC; formatado depois com `Intl.DateTimeFormat("pt-BR")` num servidor em fuso atrás de UTC (ex: America/Sao_Paulo, -03:00), a data exibida fica um dia a menos. Corrigido no campo `validade` da proposta usando `new Date(`${valor}T00:00:00`)`. **O mesmo padrão existe em `ObraForm` (data de início/término) e outros modais com campo de data** — não foi corrigido nesta rodada por estar fora do escopo pedido; vale uma rodada dedicada.

### Testado
Fluxo completo via Playwright contra o servidor local: cadastro de prestador com e-mail/telefone/categoria (persistido e exibido corretamente na lista), cadastro de obra com tipo/cidade/responsável técnico (persistido e exibido no painel Detalhes), convite de usuário (linha `User` + usuário no Supabase Auth criados, limpos depois), proposta com validade/condições de pagamento (PDF gerado com os dois campos corretos), dashboard (tendência do KPI, gráfico agregado, dark mode ida-e-volta, busca Cmd+K encontrando obra por nome). `npx tsc --noEmit` e `npm run build` limpos.
