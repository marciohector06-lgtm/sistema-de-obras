...
interface PropostasFiltrosProps {
  clientes: { id: string; nome: string }[];
}

const STATUS_OPTIONS = Object.entries(PROPOSTA_STATUS_LABELS);

export function PropostasFiltros({ clientes }: PropostasFiltrosProps) {
    ...
    <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center">
      ...
      >
        <SelectTrigger className="w-full sm:w-44">
          <SelectValue>
            {(value: string) => (!value || value === "all" ? "Todos os status" : PROPOSTA_STATUS_LABELS[value as keyof typeof PROPOSTA_STATUS_LABELS])}
          </SelectValue>
        </SelectTrigger>
        <SelectContent>
